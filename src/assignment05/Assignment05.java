...
public class Assignment05 {
	...
	public Assignment05() {
		...
		List<Item> targetShip = new ArrayList<Item>();
		...
		while (true) {
			...
			case 6:
				...
				ransack(cargohold, targetShip, getCurrentInventoryWeight(cargohold));
				break;
			case 0:
	...
	private double getCurrentInventoryWeight(List<Item> cargohold) {
	...
	private void ransack(List<Item> cargohold, List<Item> targetShip, double currentWeight) {
		if (targetShip.isEmpty()) {
			return;
		}
		// Identify and remove items that will not fit in our cargohold
		
		Iterator<Item> iter = targetShip.iterator();
		
		while (iter.hasNext()) {
			Item temp = iter.next();
			
			if (currentWeight + temp.getWeight() >= 25.0001) {
				iter.remove();
			}
		}
		...
		cargohold.add(mostValuable);
		currentWeight += mostValuable.getWeight();
		targetShip.remove(mostValuable);
		
		//Call the method recursively
		ransack(cargohold, targetShip, currentWeight);
	}
	...
	private void loadRansack(List<Item> targetShip) {
		BufferedReader br = null;
